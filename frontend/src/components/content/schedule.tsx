...
import { Index, Match, Suspense, Switch, createResource, createSignal } from "solid-js"
...
export const Schedule = () => {
    ...
    const onScheduleSettingsClick = (job) => {
        toggleModal(true)
        setModalContent(() => (
            <UpdateSchedule job={parseCronJob(job)} refetch={refetchSchedules} onClose={() => toggleModal(false)} />
        ))
    }
    ...
    const onAddScheduleClick = () => {
        toggleModal(true)
        setModalContent(() => (
            <CreateSchedule refetch={refetchSchedules} onClose={() => toggleModal(false)} />
        ))
    }
                                                ...
                                                <Index each={sensor()[1]}>
                                                    {(job, j) => {
                                                        return (<div class="w-full flex flex-row justify-between items-center">
                                                            ...
                                                            <button onClick={() => onScheduleSettingsClick(job())}>
