...
const ImageView = (props) => {
  ...
  return (
    <div class="h-[50vh] w-[50vh] p-10 bg-white rounded-sm">
      <div class="">
        <img src={props.image.url} class="object-cover mb-4" />
          ...
          <Suspense fallback={<Detail>Loading...</Detail>}>
            <Switch>
              <Match when={images_filenames()?.error}>
                <Detail
                  error
                >{`${String(images_filenames()?.error).slice(0, 30)}...`}</Detail>
              </Match>
              ...
              <Match when={images_filenames()}>
                {Object.entries(images_filenames().value).length == 0 ? (
                  <Detail>No images.</Detail>
                ) : (
                  ''
                )}
                    ...
                    return (
                      <div class="w-20 h-20 bg-zinc-200 rounded-[8px]">
                        <button
                          class="h-full"
                          onClick={() =>
                            onImageClick(image(), refetch, toggleModal)
                          }
                        >
                          <img src={image().url} class="object-cover h-full" />
                        </button>
                      </div>
                    );
                  }}
