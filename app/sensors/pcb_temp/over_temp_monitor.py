...
def gpio_callback(channel):
    if button.is_pressed:
        print("Trigger HIGH - Temperature: %.2f C" % pct.temperature)
    else:
        print("Trigger LOW - Temperature: %.2f C" % pct.temperature)
...
try:
    while True:
        if button.is_pressed:
            print("HIGH Temperature: %.2f C" % pct.temperature)
        else: 
            print("LOW Temperature: %.2f C" % pct.temperature)
        time.sleep(0.5)
