...
The camera count follows the detected model. A **Home-line** unit (1.0, 2.0, 3.0,
4.0) reports two cameras; a **Studio-line** unit (Studio, Studio 2) reports one.
Where there is no lower camera the lower-camera route returns `404`, MQTT skips
lower-camera discovery and publishing, and the web UI hides the lower-camera
controls. For a unit that differs from its profile, override it in the Pi-local
`.env`:
